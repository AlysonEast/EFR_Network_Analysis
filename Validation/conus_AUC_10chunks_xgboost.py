...
file_directory = "./xgboost_samples/chunk10/AUC/"
...
for file_name in file_list:
    if file_name.endswith('.csv'):
        ...
        model.save_model(os.path.join('./models_10chunk/AUC/', model_filename))
        ...
        plt.savefig(os.path.join('./figures_10chunk/AUC/', os.path.splitext(file_name)[0] + '_true_vs_predicted_values.png'))
        plt.close()
        ...
        plt.savefig(os.path.join('./figures_10chunk/AUC/', os.path.splitext(file_name)[0] + '_distribution_of_residuals.png'))
        plt.close()

        plt.figure(figsize=(4, 4))
        ax = plot_importance(model)
        plt.savefig(os.path.join('./figures_10chunk/AUC/', os.path.splitext(file_name)[0] + 'Var_Loadings.png'))
        plt.close()
...
preds_df = pd.concat([preds_df, overall_metrics], ignore_index=True)

# Save the results DataFrame to a CSV file
results_df.to_csv('./model_AUC_10chunk_results.csv', index=False)
preds_df.to_csv('./model_AUC_10chunk_preds.csv', index=False)
